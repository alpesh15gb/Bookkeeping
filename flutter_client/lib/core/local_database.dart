...
class LocalDatabase {
  ...
  static Future<void> cacheProducts(String tenantId, List<dynamic> products) async {
    final rows = products.map((p) => {
      ...
      'uom': p['uom']?.toString() ?? '',
      'sales_price': (p['sales_price'] as num?)?.toDouble() ?? 0.0,
      'purchase_price': (p['purchase_price'] as num?)?.toDouble() ?? 0.0,
      'gst_rate': (p['gst_rate'] as num?)?.toDouble() ?? 0.0,
      'current_stock': (p['current_stock'] as num?)?.toDouble() ?? 0.0,
      'json': jsonEncode(p),
      'synced_at': DateTime.now().toIso8601String(),
    }).toList();
  ...
  static Future<void> cacheInvoices(String tenantId, List<dynamic> invoices) async {
    final rows = invoices.map((inv) => {
      ...
      'amount_paid': (inv['amount_paid'] as num?)?.toDouble() ?? 0.0,
