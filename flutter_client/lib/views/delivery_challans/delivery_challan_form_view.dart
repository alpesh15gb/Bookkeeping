...
class _DeliveryChallanFormViewState extends State<DeliveryChallanFormView> {
  ...
  void initState() {
    ...
    if (widget.challan != null) {
      _dateCtrl.text = widget.challan!['issued_date'] ?? _dateCtrl.text;
      _notesCtrl.text = widget.challan!['notes'] ?? '';
      for (final item in (widget.challan!['lines'] as List? ?? [])) {
  ...
  void _addLine(ProductModel product) {
    setState(() => _lines.add(_DCLineItem(productId: product.id, productName: product.name)));
  }
              ...
              child: Column(
                children: [
                  ...
                  TextFormField(
                    ...
                    readOnly: true,
                    decoration: const InputDecoration(labelText: 'Issue Date *', prefixIcon: Icon(Icons.calendar_today, size: 16)),
                    onTap: _pickDate,
...
class _DCLineItem {
  ...
  _DCLineItem({required this.productId, required this.productName, this.quantity = 1}) {
