...
class _ProfitLossViewState extends State<ProfitLossView> {
  ...
  Future<void> _downloadPdf() async {
    ...
    final start = '2020-04-01'; // Default FY start
    final today = DateTime.now();
    final end = '${today.year}-${today.month.toString().padLeft(2, '0')}-${today.day.toString().padLeft(2, '0')}';
    final url = Uri.parse(
      '${ApiClient.baseUrl}/reports/profit-loss/pdf'
  ...
  Future<void> _downloadExcel() async {
    ...
    final end = '${today.year}-${today.month.toString().padLeft(2, '0')}-${today.day.toString().padLeft(2, '0')}';
    final url = Uri.parse(
      '${ApiClient.baseUrl}/reports/profit-loss/excel'
      '?start_date=$start&end_date=$end'
      '&token=$token&tenant_id=$tenantId',
    );
    if (await canLaunchUrl(url)) {
