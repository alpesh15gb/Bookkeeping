...
class AccountingProvider extends ChangeNotifier {
  ...
  Future<List<dynamic>> fetchJournals() async {
    ...
    try {
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/accounting/journals'),
      );
  ...
  Future<Map<String, dynamic>?> fetchLedgerStatement(String accountId) async {
    ...
    try {
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/accounting/ledger/$accountId'),
      );
      if (response.statusCode == 200) {
        _isLoading = false;
        notifyListeners();
  ...
  Future<Map<String, dynamic>?> fetchTrialBalance() async {
    ...
    try {
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/accounting/trial-balance'),
      );
      if (response.statusCode == 200) {
        _isLoading = false;
        notifyListeners();
  ...
  Future<Map<String, dynamic>?> fetchProfitLoss() async {
    ...
    try {
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/accounting/profit-loss'),
      );
      if (response.statusCode == 200) {
        _isLoading = false;
        notifyListeners();
  ...
  Future<Map<String, dynamic>?> fetchBalanceSheet() async {
    ...
    try {
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/accounting/balance-sheet'),
      );
      if (response.statusCode == 200) {
        _isLoading = false;
        notifyListeners();
  ...
  Future<void> fetchAccounts() async {
    ...
    try {
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/masters/accounts'),
      );
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
  ...
  Future<bool> updateAccount(String id, Map<String, dynamic> payload) async {
    ...
    try {
      final response = await _client.put(
        _buildUri('${ApiClient.baseUrl}/masters/accounts/$id'),
        body: jsonEncode(payload),
      );
      if (response.statusCode == 200) {
        await fetchAccounts();
  ...
  Future<Map<String, dynamic>?> fetchSalesAnalytics({
    ...
    try {
      ...
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/reports/analytics/sales$query'),
      );
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return data is Map<String, dynamic> ? data : (data is Map ? Map<String, dynamic>.from(data) : null);
  ...
  Future<Map<String, dynamic>?> fetchPurchaseAnalytics({
    ...
    try {
      ...
      final query = params.isNotEmpty ? '?${params.join('&')}' : '';
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/reports/analytics/purchases$query'),
      );
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return data is Map<String, dynamic> ? data : (data is Map ? Map<String, dynamic>.from(data) : null);
  ...
  Future<Map<String, dynamic>?> fetchOutstandingReceivables({
    ...
    try {
      ...
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/reports/outstanding/receivables$query'),
      );
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return data is Map<String, dynamic> ? data : (data is Map ? Map<String, dynamic>.from(data) : null);
  ...
  Future<Map<String, dynamic>?> fetchOutstandingPayables({
    ...
    try {
      final query = asOfDate != null ? '?as_of_date=$asOfDate' : '';
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/reports/outstanding/payables$query'),
      );
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return data is Map<String, dynamic> ? data : (data is Map ? Map<String, dynamic>.from(data) : null);
  ...
  Future<Map<String, dynamic>?> fetchYearEndPrepare(String closingDate) async {
    ...
    try {
      final response = await _client.get(
        _buildUri('${ApiClient.baseUrl}/accounting/year-end/prepare?closing_date=$closingDate'),
      );
      if (response.statusCode == 200) {
        _isLoading = false;
        notifyListeners();
  ...
  Future<Map<String, dynamic>?> closeYearEnd(String closingDate) async {
    ...
    try {
      final response = await _client.post(
        _buildUri('${ApiClient.baseUrl}/accounting/year-end/close'),
