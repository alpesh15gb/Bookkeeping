    ...
    return ApexCard(
      ...
      child: Column(
        ...
        children: [
          ...
          // Table — sized to content; the overview scroll view handles
          // vertical scrolling.
          SingleChildScrollView(
            ...
            child: DataTable(
              ...
              columns: [
                ...
                DataColumn(
                  label: _headerCell(context, 'Description', width: 280),
                ),
                DataColumn(label: _headerCell(context, 'HSN/SAC', width: 100)),
                DataColumn(
                  label: _headerCell(context, 'Qty', width: 80),
                  numeric: true,
                ),
                DataColumn(label: _headerCell(context, 'Unit', width: 80)),
                DataColumn(
                  label: _headerCell(context, 'Rate', width: 120),
                  numeric: true,
                ),
                DataColumn(
                  label: _headerCell(context, 'Disc %', width: 100),
                  numeric: true,
                ),
                DataColumn(
                  label: _headerCell(context, 'GST %', width: 90),
                  numeric: true,
                ),
                DataColumn(
                  label: _headerCell(context, 'Amount', width: 140),
                  numeric: true,
                ),
              ],
              rows: List.generate(lines.length, (index) {
                ...
                return DataRow(
                  ...
                  cells: [
                    ...
                    DataCell(
                      Text(
                        (line.unit?.isNotEmpty ?? false) ? line.unit! : 'PCS',
                        style: textTheme.bodyMedium?.copyWith(
                          color: colors.textPrimary,
                        ),
                      ),
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      _DetailItem(
                        label: 'HSN/SAC',
                        value: line.hsnSac.isNotEmpty ? line.hsnSac : '—',
                        colors: colors,
                        textTheme: textTheme,
                      ),
                      _DetailItem(
                        label: 'Qty',
                        value: fmt.decimal(line.quantity),
                        colors: colors,
                        textTheme: textTheme,
                      ),
                      _DetailItem(
                        label: 'Unit',
                        value: (line.unit?.isNotEmpty ?? false)
                            ? line.unit!
                            : 'PCS',
                        colors: colors,
                        textTheme: textTheme,
                      ...
                      _DetailItem(
                        label: 'Disc %',
                        value: '${line.discount.toStringAsFixed(2)}%',
                        colors: colors,
                        textTheme: textTheme,
                      ),
                      _DetailItem(
    ...
    if (invoice.lines.isNotEmpty) {
      for (final line in invoice.lines) {
        final taxable =
            ...
            (line.quantity * line.rate * (line.discount / 100));
        final taxAmount = taxable * (line.gstRate / 100);
