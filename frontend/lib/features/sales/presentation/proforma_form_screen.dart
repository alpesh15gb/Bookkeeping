/// Proforma Invoice (Quotation) form with customer autocomplete, line items,
/// dates, and totals.
library;
...
class ProformaFormNotifier extends StateNotifier<ProformaFormState> {
  ...
  void _recalc(ProformaFormState s) {
    final r = _calculateAll(s.lines);
    state = s.copyWith(
      lines: r.lines,
      subtotal: r.subtotal,
  ...
  Future<bool> create() async {
    ...
    try {
      await _dio.post('/proforma-invoices', data: _buildPayload());
...
final proformaFormProvider =
    StateNotifierProvider.autoDispose<ProformaFormNotifier, ProformaFormState>(
      (ref) => ProformaFormNotifier(ref.watch(apiClientProvider)),
    );
...
class ProformaFormScreen extends ConsumerStatefulWidget {
  const ProformaFormScreen({super.key, this.editId});
  final String? editId;
...
class _ProformaFormScreenState extends ConsumerState<ProformaFormScreen> {
  ...
  bool get _hasUnsavedChanges {
    final s = ref.read(proformaFormProvider);
    return s.contactId != null || s.lines.any((l) => l.productId.isNotEmpty);
  }

  Future<void> _save() async {
    final notifier = ref.read(proformaFormProvider.notifier);
    if (ref.read(proformaFormProvider).saving) return;
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(proformaFormProvider);
    final notifier = ref.read(proformaFormProvider.notifier);
        ...
        child: PopScope(
          ...
          child: Scaffold(
            ...
            appBar: AppBar(
              ...
              title: Text(
                widget.editId != null ? 'Edit Quotation' : 'New Quotation',
                style: TextStyle(
              ...
              actions: [
                ...
                Padding(
                  ...
                  child: FilledButton.icon(
                    ...
                    label: Text(
                      widget.editId != null ? 'Update' : 'Save draft',
                    ),
  ...
  Widget _headerCard(
    ProformaFormState state,
    ProformaFormNotifier notifier,
    ApexColors colors,
