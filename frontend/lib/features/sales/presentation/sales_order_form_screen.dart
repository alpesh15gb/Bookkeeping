/// Sales Order form with customer, line items, and confirm workflow.
library;
...
class SalesOrderFormNotifier extends StateNotifier<SalesOrderFormState> {
  ...
  void _recalc(SalesOrderFormState s) {
    final r = _soCalculateAll(s.lines);
    state = s.copyWith(
      lines: r.lines,
      subtotal: r.subtotal,
  ...
  Future<bool> create() async {
    ...
    try {
      await _dio.post('/sales-orders', data: _buildPayload());
  ...
  Future<bool> update(String id) async {
    state = state.copyWith(saving: true, error: null, clearError: true);
    try {
      await _dio.put('/sales-orders/$id', data: _buildPayload());
      state = state.copyWith(saving: false);
      return true;
    } on DioException catch (e) {
      state = state.copyWith(
...
class SalesOrderFormScreen extends ConsumerStatefulWidget {
  const SalesOrderFormScreen({super.key, this.editId});
  final String? editId;
...
class _SalesOrderFormScreenState extends ConsumerState<SalesOrderFormScreen> {
  ...
  void initState() {
    ...
    Future.microtask(() {
      ...
      if (widget.editId != null) {
        ref.read(salesOrderFormProvider.notifier).loadForEdit(widget.editId!);
      } else {
        final n = ref.read(salesOrderFormProvider.notifier);
  ...
  bool get _hasUnsavedChanges {
    final s = ref.read(salesOrderFormProvider);
    return s.contactId != null || s.lines.any((l) => l.productId.isNotEmpty);
  }

  Future<void> _save() async {
    final notifier = ref.read(salesOrderFormProvider.notifier);
    if (ref.read(salesOrderFormProvider).saving) return;
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(salesOrderFormProvider);
    final notifier = ref.read(salesOrderFormProvider.notifier);
        ...
        child: PopScope(
          ...
          child: Scaffold(
            ...
            appBar: AppBar(
              ...
              title: Text(
                widget.editId != null ? 'Edit Order' : 'New Sales Order',
                style: TextStyle(
  ...
  Widget _headerCard(
    SalesOrderFormState state,
    SalesOrderFormNotifier notifier,
    ApexColors colors,
