/// Bill calculation service — pure math, mirrors InvoiceCalculationService.
library;
...
class BillCalculationService {
  ...
  /// Calculate TDS amount based on TDS rate and bill total.
  double calculateTds({required double total, required double tdsRate}) =>
      round2(total * (tdsRate / 100));

  /// Calculate round-off to bring the total to whole rupees.
  double calculateRoundOff({required double rawTotal}) {
    final rounded = rawTotal.roundToDouble();
    return round2(rounded - rawTotal);
  }
