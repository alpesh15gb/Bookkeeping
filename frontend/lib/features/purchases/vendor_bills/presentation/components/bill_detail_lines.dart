/// Vendor Bill Detail Lines — Read-only table with expandable tax details.
library;
...
class BillDetailLines extends StatelessWidget {
  const BillDetailLines({super.key, required this.bill, required this.fmt});
    ...
    return ApexCard(
      ...
      child: Column(
        ...
        children: [
          ...
          // Tax Summary Footer
          _TaxSummaryFooter(bill: bill, fmt: fmt),
        ],
      ),
    );
  }

  Widget _buildMobileCards(BuildContext context, List<BillLine> lines) {
    final colors = apexColors(context);
    final textTheme = Theme.of(context).textTheme;
  ...
  Widget _buildDescriptionCell(BillLine line, TextTheme textTheme, ApexColors colors) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
  ...
  String _getTaxLabel(VendorBill bill, BillLine line) {
    if (bill.igstAmount > 0 && bill.cgstAmount == 0 && bill.sgstAmount == 0) {
      return 'IGST';
    } else if (bill.cgstAmount > 0 || bill.sgstAmount > 0) {
      return 'CGST + SGST';
    } else if (bill.utgstAmount > 0) {
      return 'UTGST';
    } else {
      return 'GST';
    }
...
class _TaxSummaryFooter extends StatelessWidget {
  const _TaxSummaryFooter({required this.bill, required this.fmt});
