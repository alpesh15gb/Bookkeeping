/// Customer Ledger screen — party statement for a selected customer.
///
/// Shows all transactions (invoices, payments, returns, etc.) for a customer
...
final clDateFromProvider = StateProvider<String?>((ref) => null);
...
class CustomerSearchDelegate extends SearchDelegate<ContactSummary?> {
  CustomerSearchDelegate(this.reportsService, this.colors) : super();
...
class _CustomerLedgerScreenState extends ConsumerState<CustomerLedgerScreen> {
  DateTime? _fromDate;
  DateTime? _toDate;
  ContactSummary? _selectedCustomer;
  ...
  void initState() {
    ...
    _toDate = now;
    Future.microtask(() {
      ref.read(clDateFromProvider.notifier).state = _toApiDate(_fromDate!);
      ref.read(clDateToProvider.notifier).state = _toApiDate(_toDate!);
    });
  }

  @override
  Widget build(BuildContext context) {
    final asyncVal = ref.watch(customerLedgerProvider);
      ...
      body: Column(
        children: [
          PageHeader(
            ...
            subtitle: 'Transaction history for a selected customer.',
          ...
          Padding(
            ...
            child: _buildCustomerSelector(colors),
          ),
          Expanded(
            child: asyncVal.when(
  ...
  Widget _buildCustomerSelector(ApexColors colors) {
    return InkWell(
      onTap: () async {
        ...
        if (result != null) {
          ...
          ref.read(clContactIdProvider.notifier).state = result.id;
        }
      },
            ...
            if (_selectedCustomer != null)
              GestureDetector(
                onTap: () {
                  ...
                  ref.read(clContactIdProvider.notifier).state = null;
                },
                child: Icon(
                  Icons.close_rounded,
                  size: 16,
  ...
  Future<void> _pickDate({required bool isFrom}) async {
    ...
    setState(() {
      if (isFrom) {
        _fromDate = picked;
        ref.read(clDateFromProvider.notifier).state = _toApiDate(picked);
      } else {
        _toDate = picked;
        ref.read(clDateToProvider.notifier).state = _toApiDate(picked);
      }
    });
  }
    ...
    return ApexCard(
      child: Column(
        ...
        children: [
          ...
          _summaryRow(
            'Total Receipts',
            fmt.currency(summary.totalReceipts),
            colors,
          ),
