/// Bank Book report screen — mirrors the Cash Book pattern filtered to bank
/// accounts.
///
/// Displays a chronological register of all bank account transactions with
...
final bankBookReportProvider = FutureProvider.autoDispose<CashBookReport>((
  ...
  final res = await ref
      ...
      .getBankBook(startDate: dateFrom, endDate: dateTo);
  return switch (res) {
...
class BankBookScreen extends ConsumerStatefulWidget {
  const BankBookScreen({super.key});
  @override
  ConsumerState<BankBookScreen> createState() => _BankBookScreenState();
}

class _BankBookScreenState extends ConsumerState<BankBookScreen> {
  DateTime? _fromDate;
  DateTime? _toDate;
  ...
  Widget build(BuildContext context) {
    final asyncVal = ref.watch(bankBookReportProvider);
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          PageHeader(
            title: 'Bank Book',
            subtitle: 'Bank transaction register for the period.',
  ...
  Future<void> _pickDate({required bool isFrom}) async {
    ...
    setState(() {
      if (isFrom) {
        _fromDate = picked;
        ref.read(bankBookDateFromProvider.notifier).state = _toApiDate(picked);
      } else {
        _toDate = picked;
        ref.read(bankBookDateToProvider.notifier).state = _toApiDate(picked);
      }
    });
  }
