...
interface PurchaseOrderDetailProps {
  poId: string;
  onNavigate: (view: "purchase_orders" | "purchase_order_create" | "purchase_order_detail", poId?: string) => void;
}
...
export default function PurchaseOrderDetail({ poId, onNavigate }: PurchaseOrderDetailProps) {
  ...
  const { data: companyData } = useQuery({
    queryKey: ["company", po?.tenant_id],
    queryFn: async () => {
      const res = await apiClient.get(`/companies/${po.tenant_id}`);
      return res.data;
    },
    enabled: !!po?.tenant_id,
  });
  ...
  const { data: printData } = useQuery({
    queryKey: ["po-print", poId],
    queryFn: async () => {
      const res = await apiClient.get(`/purchase-orders/${poId}/pdf-payload`);
      return res.data;
    },
  ...
  useEffect(() => {
    if (printData && showPrintModal) {
      ...
      const docNumber = printData.purchase_order?.po_number || "";
      const html = `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8"/>
<title>Purchase Order ${docNumber}</title>
<style>
...
<div class="meta-grid">
  <div class="item"><div class="label">Order Date</div><div class="value">${printData.purchase_order?.order_date ? new Date(printData.purchase_order.order_date).toLocaleDateString("en-IN") : ""}</div></div>
  <div class="item"><div class="label">Due Date</div><div class="value">${printData.purchase_order?.due_date ? new Date(printData.purchase_order.due_date).toLocaleDateString("en-IN") : ""}</div></div>
  <div class="item"><div class="label">Place of Supply</div><div class="value">${printData.purchase_order?.pos_state_code || ""}</div></div>
  <div class="item"><div class="label">Status</div><div class="value">${printData.purchase_order?.status || ""}</div></div>
</div>
...
<table class="totals">
  <tr><td class="label-cell">Subtotal</td><td class="value-cell">${formatCurrency(printData.purchase_order?.subtotal || 0)}</td></tr>
  ${(printData.purchase_order?.discount_total || 0) > 0 ? `<tr><td class="label-cell">Discount</td><td class="value-cell">-${formatCurrency(printData.purchase_order.discount_total)}</td></tr>` : ""}
  ${(printData.purchase_order?.cgst_amount || 0) > 0 ? `<tr><td class="label-cell">CGST</td><td class="value-cell">${formatCurrency(printData.purchase_order.cgst_amount)}</td></tr>` : ""}
  ${(printData.purchase_order?.sgst_amount || 0) > 0 ? `<tr><td class="label-cell">SGST</td><td class="value-cell">${formatCurrency(printData.purchase_order.sgst_amount)}</td></tr>` : ""}
  ${(printData.purchase_order?.igst_amount || 0) > 0 ? `<tr><td class="label-cell">IGST</td><td class="value-cell">${formatCurrency(printData.purchase_order.igst_amount)}</td></tr>` : ""}
  <tr class="grand-total"><td class="label-cell">Total</td><td class="value-cell">${formatCurrency(printData.purchase_order?.total || 0)}</td></tr>
</table>

<div class="footer">
  <p>This is a computer-generated purchase order and does not require a physical signature.</p>
</div>
  ...
  if (error || !po) {
    return (
      <div className="flex items-center gap-3 p-4 bg-rose-50 border border-rose-200 text-rose-700 rounded-lg">
        ...
        <span>Error retrieving purchase order details.</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center no-print">
        <div className="flex items-center gap-3">
          ...
          <span className={`px-2.5 py-0.5 text-xs font-semibold rounded-full border ${getStatusColor(po.status)}`}>
            {po.status}
          </span>
        </div>
        <button
      ...
      <div className="bg-white p-8 rounded-xl border border-slate-100 shadow-sm print-box">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-8 border-b border-slate-100">
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Vendor</span>
            <h3 className="text-lg font-bold text-slate-900 mt-1">{po.contact.name}</h3>
            {po.contact.billing_address && (
              <div className="text-sm text-slate-500 mt-1">
                <p>{po.contact.billing_address.street}</p>
                <p>{po.contact.billing_address.city}, {po.contact.billing_address.state} - {po.contact.billing_address.pincode}</p>
              </div>
            )}
            {po.contact.gstin && <p className="text-sm font-semibold text-slate-700 mt-2">GSTIN: {po.contact.gstin}</p>}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-8 border-b border-slate-100 text-sm">
          ...
          <div>
            ...
            <span className="font-semibold text-slate-700 mt-1 block">
              {new Date(po.due_date).toLocaleDateString("en-IN")}
            </span>
          </div>
          <div>
            <span className="text-slate-400 font-semibold block uppercase text-[10px] tracking-wider">Place of Supply</span>
            <span className="font-semibold text-slate-700 mt-1 block font-mono">State Code: {po.pos_state_code}</span>
          </div>
        </div>
        ...
        <div className="space-y-3 md:max-w-md md:ml-auto w-full text-sm mt-8 pt-8 border-t border-slate-100">
          <div className="flex justify-between text-slate-500">
            <span>Subtotal</span>
            <span>{formatCurrency(parseFloat(po.subtotal))}</span>
          </div>
          {parseFloat(po.discount_total) > 0 && (
            <div className="flex justify-between text-slate-500">
              <span>Discount</span>
              <span className="text-rose-600">-{formatCurrency(parseFloat(po.discount_total))}</span>
            </div>
          )}
          {parseFloat(po.cgst_amount) > 0 && (
            <div className="flex justify-between text-slate-400 italic pl-2 text-xs">
              <span>CGST</span>
              <span>{formatCurrency(parseFloat(po.cgst_amount))}</span>
            </div>
          )}
          {parseFloat(po.sgst_amount) > 0 && (
            <div className="flex justify-between text-slate-400 italic pl-2 text-xs">
              <span>SGST</span>
              <span>{formatCurrency(parseFloat(po.sgst_amount))}</span>
            </div>
          )}
          {parseFloat(po.igst_amount) > 0 && (
            <div className="flex justify-between text-slate-400 italic pl-2 text-xs">
              <span>IGST</span>
              <span>{formatCurrency(parseFloat(po.igst_amount))}</span>
            </div>
          )}
          <div className="flex justify-between text-base font-bold text-slate-900 pt-2 border-t border-slate-100">
            <span>Total</span>
            <span>{formatCurrency(parseFloat(po.total))}</span>
          </div>
        </div>
      </div>
    </div>
