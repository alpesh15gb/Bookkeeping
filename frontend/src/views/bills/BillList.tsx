...
import { Plus, Search, Receipt, Eye, Edit, ShieldAlert, SlidersHorizontal, ChevronDown } from "lucide-react";
...
export default function BillList({ onNavigate }: BillListProps) {
  ...
  const filteredBills = bills.filter((bill) => {
    ...
    if (statusFilter === "PAID") return matchesSearch && bill.status.toUpperCase() === "PAID";
    return matchesSearch && bill.status.toUpperCase() !== "PAID";
  });
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-2">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">Manage purchase bills and vendor payments.</p>
        </div>
        <button
      ...
      {isLoading ? (
        ...
        <div className="flex items-center gap-3 p-4 bg-rose-50 border border-rose-200 text-rose-700 rounded-lg">
          ...
          <span className="text-xs font-semibold">Error loading vendor bills. Please check API server.</span>
        </div>
        ...
        <div className="text-center py-16 bg-white rounded-xl border border-zinc-200 shadow-sm">
          ...
          <p className="text-xs text-zinc-400 mt-1">Try resetting filters or record a new bill to get started.</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm border border-zinc-200 overflow-hidden">
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-zinc-50 text-zinc-500 border-b border-zinc-100 font-semibold">
                  <th className="px-4 py-3 font-semibold uppercase tracking-wider">Bill Number</th>
                  <th className="px-4 py-3 font-semibold uppercase tracking-wider">Vendor</th>
                  <th className="px-4 py-3 font-semibold uppercase tracking-wider">Issue Date</th>
              ...
              <tbody className="divide-y divide-zinc-100">
                {paginatedBills.map((b) => (
                  <tr key={b.id} className="hover:bg-zinc-50/40 transition">
                    <td className="px-4 py-3.5 font-mono font-medium text-zinc-900">{b.bill_number}</td>
                    <td className="px-4 py-3.5 font-bold text-zinc-850">{b.contact_name}</td>
                    ...
                    <td className="px-4 py-3.5 text-right font-mono font-medium text-zinc-600">
                      {formatCurrency(b.total - b.amount_paid)}
                    </td>
          ...
          <div className="px-6 py-4 border-t border-zinc-100 flex items-center justify-between">
            <span className="text-xs text-zinc-500 font-medium">
              Showing {totalItems > 0 ? startIndex + 1 : 0} to {Math.min(startIndex + itemsPerPage, totalItems)} of {totalItems} bills
            </span>
