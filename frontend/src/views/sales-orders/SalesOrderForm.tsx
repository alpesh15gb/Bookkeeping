...
export default function SalesOrderForm({ editId, onNavigate, onSuccess }: SalesOrderFormProps) {
  const isEdit = !!editId;
  ...
  // Fetch Customer contacts from API
  ...
  const customers = contacts.filter(c => c.contact_type === "CUSTOMER" || c.contact_type === "BOTH");
  ...
  // Fetch SO details if editing
  const { data: so } = useQuery({
    queryKey: ["sales-order", editId],
    queryFn: async () => {
      const res = await apiClient.get(`/sales-orders/${editId}`);
      return res.data;
    },
    enabled: isEdit,
  });
  ...
  const handleCustomerChange = (selectedContactId: string) => {
    ...
    const selected = customers.find((c) => c.id === selectedContactId);
    if (selected) {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (isEdit) {
        return apiClient.put(`/sales-orders/${editId}`, payload);
      } else {
        return apiClient.post("/sales-orders", payload);
      }
    },
    onSuccess: () => {
      onSuccess();
    ...
    onError: (err: any) => {
      const msg = err.response?.data?.detail || "Failed to save sales order. Check API inputs.";
      setFormError(msg);
    }
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!soNumber) {
      setFormError("SO number is required.");
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
          ...
          {/* SO Number */}
          <div className="space-y-2">
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider">Sales Order Number</label>
            <input
              type="text"
              value={soNumber}
          ...
          {/* Due Date */}
          <div className="space-y-2">
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider">Expected Delivery Date</label>
            <input
              type="date"
              value={dueDate}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-xl border border-slate-100 shadow-sm md:col-span-2">
            <h3 className="font-semibold text-sm text-slate-700 mb-2">Terms & Conditions</h3>
            <textarea
              rows={4}
              placeholder="Record delivery terms, payment terms or special agreements..."
