...
export default function CreditNoteDetail({ cnId, onNavigate }: CreditNoteDetailProps) {
  ...
  const { data: cn, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.get(`/invoices/credit-notes/${cnId}`);
      return res.data;
    },
  });

  const { data: companyData } = useQuery({
    queryKey: ["company", cn?.tenant_id],
    queryFn: async () => {
      const res = await apiClient.get(`/companies/${cn.tenant_id}`);
      return res.data;
    },
    enabled: !!cn?.tenant_id,
  });
  ...
  const { data: printData } = useQuery({
    queryKey: ["cn-print", cnId],
    queryFn: async () => {
      const res = await apiClient.get(`/invoices/credit-notes/${cnId}/pdf-payload`);
      return res.data;
    },
  ...
  useEffect(() => {
    if (printData && showPrintModal) {
      ...
      const docNumber = printData.credit_note?.credit_note_number || "";
      const html = `<!DOCTYPE html>
<html>
<head>
<meta charset="utf-8"/>
<title>Credit Note ${docNumber}</title>
<style>
...
<div class="meta-grid">
  <div class="item"><div class="label">Issue Date</div><div class="value">${printData.credit_note?.issue_date ? new Date(printData.credit_note.issue_date).toLocaleDateString("en-IN") : ""}</div></div>
  <div class="item"><div class="label">Place of Supply</div><div class="value">${printData.credit_note?.pos_state_code || ""}</div></div>
  <div class="item"><div class="label">Status</div><div class="value">${printData.credit_note?.status || ""}</div></div>
  ${printData.credit_note?.reason ? `<div class="item"><div class="label">Reason</div><div class="value">${printData.credit_note.reason}</div></div>` : ""}
</div>

<table>
  <thead><tr>
    <th>#</th><th>Item</th><th>HSN/SAC</th><th class="text-right">Qty</th>
    <th class="text-right">Rate</th><th class="text-right">Tax</th><th class="text-right">Amount</th>
  </tr></thead>
  <tbody>
...
<table class="totals">
  <tr><td class="label-cell">Subtotal</td><td class="value-cell">${formatCurrency(printData.credit_note?.subtotal || 0)}</td></tr>
  ${(printData.credit_note?.cgst_amount || 0) > 0 ? `<tr><td class="label-cell">CGST</td><td class="value-cell">${formatCurrency(printData.credit_note.cgst_amount)}</td></tr>` : ""}
  ${(printData.credit_note?.sgst_amount || 0) > 0 ? `<tr><td class="label-cell">SGST</td><td class="value-cell">${formatCurrency(printData.credit_note.sgst_amount)}</td></tr>` : ""}
  ${(printData.credit_note?.igst_amount || 0) > 0 ? `<tr><td class="label-cell">IGST</td><td class="value-cell">${formatCurrency(printData.credit_note.igst_amount)}</td></tr>` : ""}
  ${(printData.credit_note?.round_off || 0) !== 0 ? `<tr><td class="label-cell">Round Off</td><td class="value-cell">${formatCurrency(printData.credit_note.round_off)}</td></tr>` : ""}
  <tr class="grand-total"><td class="label-cell">Total</td><td class="value-cell">${formatCurrency(printData.credit_note?.total || 0)}</td></tr>
</table>
  ...
  if (error || !cn) {
    return (
      <div className="flex items-center gap-3 p-4 bg-rose-50 border border-rose-200 text-rose-700 rounded-lg">
        ...
        <span>Error retrieving credit note details.</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center no-print">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-slate-900 font-mono">{cn.credit_note_number}</h1>
          <span className={`px-2.5 py-0.5 text-xs font-semibold rounded-full border ${getStatusColor(cn.status)}`}>
            {cn.status}
          </span>
        </div>
        <button
      ...
      <div className="bg-white p-8 rounded-xl border border-slate-100 shadow-sm print-box">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-8 border-b border-slate-100">
          ...
          <div className="md:text-right">
            ...
            <h3 className="text-lg font-bold text-slate-900 mt-1">{cn.invoice?.contact?.name || "—"}</h3>
            {cn.invoice?.contact?.gstin && <p className="text-sm font-semibold text-slate-700 mt-2">GSTIN: {cn.invoice.contact.gstin}</p>}
          </div>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 py-8 border-b border-slate-100 text-sm">
          <div>
            <span className="text-slate-400 font-semibold block uppercase text-[10px] tracking-wider">Issue Date</span>
            <span className="font-semibold text-slate-700 mt-1 block">
              {new Date(cn.issue_date).toLocaleDateString("en-IN")}
            </span>
          </div>
          <div>
            <span className="text-slate-400 font-semibold block uppercase text-[10px] tracking-wider">Place of Supply</span>
            <span className="font-semibold text-slate-700 mt-1 block font-mono">State Code: {cn.pos_state_code}</span>
          </div>
          {cn.reason && (
            <div>
              <span className="text-slate-400 font-semibold block uppercase text-[10px] tracking-wider">Reason</span>
              <span className="font-semibold text-slate-700 mt-1 block">{cn.reason}</span>
            </div>
          )}
        </div>
        ...
        <table className="w-full border-collapse text-left text-sm mt-8">
          ...
          <tbody className="divide-y divide-slate-100">
            {cn.lines.map((line: any, idx: number) => (
        ...
        <div className="space-y-3 md:max-w-md md:ml-auto w-full text-sm mt-8 pt-8 border-t border-slate-100">
          <div className="flex justify-between text-slate-500">
            <span>Subtotal</span>
            <span>{formatCurrency(parseFloat(cn.subtotal))}</span>
          </div>
          {parseFloat(cn.cgst_amount) > 0 && (
            <div className="flex justify-between text-slate-400 italic pl-2 text-xs">
              <span>CGST</span>
              <span>{formatCurrency(parseFloat(cn.cgst_amount))}</span>
            </div>
          )}
          {parseFloat(cn.sgst_amount) > 0 && (
            <div className="flex justify-between text-slate-400 italic pl-2 text-xs">
              <span>SGST</span>
              <span>{formatCurrency(parseFloat(cn.sgst_amount))}</span>
            </div>
          )}
          {parseFloat(cn.igst_amount) > 0 && (
            <div className="flex justify-between text-slate-400 italic pl-2 text-xs">
              <span>IGST</span>
              <span>{formatCurrency(parseFloat(cn.igst_amount))}</span>
            </div>
          )}
          {parseFloat(cn.round_off) !== 0 && (
            <div className="flex justify-between text-slate-400 italic pl-2 text-xs">
              <span>Round Off</span>
              <span>{formatCurrency(parseFloat(cn.round_off))}</span>
            </div>
          )}
          <div className="flex justify-between text-base font-bold text-slate-900 pt-2 border-t border-slate-100">
            <span>Total</span>
            <span>{formatCurrency(parseFloat(cn.total))}</span>
          </div>
        </div>
      </div>
    </div>
